...
The Python SDK for interacting with the Thaura.ai API is [`openai`](https://pypi.org/project/openai/), as Thaura provides an OpenAI-compatible API. The Thaura implementation leverages the same OpenAI SDK but points to the Thaura.ai backend at `https://backend.thaura.ai/v1`.

Thaura.ai is an ethical, privacy-first AI platform designed for secure knowledge management with encryption. The platform provides an OpenAI-compatible API that can be used as a drop-in replacement for OpenAI endpoints. To use the Thaura API, you need to:

1. Create an account at [thaura.ai](https://thaura.ai)
2. Generate an API key from your account dashboard
3. Add funds to your account balance to use the API

The Thaura integration reuses the OpenAI LLM implementation, ensuring consistency in behavior and reducing maintenance overhead.

#### Chain-of-Thought Reasoning

Thaura's model employs chain-of-thought reasoning, which means it may wrap its internal reasoning process in `<think>...</think>` tags before providing the final SQL query. The Thaura implementation automatically handles this by:

1. Detecting the presence of `<think>` and `</think>` tags in the response
2. Stripping the chain-of-thought reasoning content
3. Extracting the clean SQL query from the remaining text

This ensures users receive only the SQL query in the editor, while the model still benefits from reasoning through the problem step-by-step internally. The implementation also handles responses formatted with markdown code blocks (` ```sql ... ``` `), applying both tag removal and markdown trimming as needed.

#### Cache

Like OpenAI, Thaura.ai does not provide explicit cache management APIs. The complete database metadata context and system instructions are included with every API call. However, similar to OpenAI, placing the context and system prompt at the beginning of the input prompt can help optimize performance if the provider implements any automatic caching mechanisms.
